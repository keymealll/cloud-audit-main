...
def check_key_rotation(provider: GCPProvider) -> CheckResult:
    ...
    try:
        ...
        locations = provider.kms_service.projects().locations().list(
            name=f"projects/{provider.project}"
        ).execute()
        ...
        for location in locations.get("locations", []):
            ...
            try:
                key_rings = provider.kms_service.projects().locations().keyRings().list(
                    parent=parent
                ).execute()
            except Exception:
                continue
            ...
            for ring in key_rings.get("keyRings", []):
                ...
                try:
                    keys = provider.kms_service.projects().locations().keyRings().cryptoKeys().list(
                        parent=ring_name
                    ).execute()
                except Exception:
                    continue

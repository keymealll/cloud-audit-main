...
class Remediation(BaseModel):
    """Remediation details for a finding - gcloud CLI command, Terraform HCL, and docs link."""

    cli: str = Field(description="gcloud CLI command (copy-paste ready)")
    terraform: str = Field(description="Terraform HCL snippet")
    doc_url: str = Field(description="Link to GCP documentation")
...
class Finding(BaseModel):
    ...
    severity: Severity
    category: Category
    resource_type: str = Field(description="GCP resource type, e.g. 'compute.googleapis.com/Instance'")
    resource_id: str = Field(description="Resource identifier (full resource name, ID, or path)")
    ...
    compliance_refs: list[str] = Field(
        ...
        description="Compliance references, e.g. ['ISO 27001 A.9.2.3', 'SOC 2 CC6.1', 'CIS 1.1']",
    )

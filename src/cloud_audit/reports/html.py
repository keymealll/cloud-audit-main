...
def render_html(report: ScanReport) -> str:
    ...
    # Collect unique compliance references across all findings
    cis_controls: list[str] = sorted(
        {ref for f in report.all_findings for ref in f.compliance_refs if ref.startswith("CIS")}
    )
    iso27001_controls: list[str] = sorted(
        {ref for f in report.all_findings for ref in f.compliance_refs if ref.startswith("ISO")}
    )
    soc2_controls: list[str] = sorted(
        {ref for f in report.all_findings for ref in f.compliance_refs if ref.startswith("SOC")}
    )
